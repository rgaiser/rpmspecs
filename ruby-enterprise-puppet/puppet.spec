...
%define ruby_bindir %(%{ruby} -rrbconfig -e "puts Config::CONFIG['bindir']")
%define ruby_sbindir %(%{ruby} -rrbconfig -e "puts Config::CONFIG['sbindir']")
%define ruby_mandir %(%{ruby} -rrbconfig -e "puts Config::CONFIG['mandir']")
...
Source2:        puppet-no-hardcoded-ruby.patch
...
BuildRequires:  ruby-enterprise >= 1.8.1
...
Requires:       facter >= 1.5
Requires:       ruby-enterprise >= 1.8.1
...
# Move the files inside the ruby directory to the system directory
mkdir -p %{buildroot}/%{_bindir}
mkdir -p %{buildroot}/%{_sbindir}
mkdir -p %{buildroot}/%{_mandir}
mv %{buildroot}/%{ruby_bindir}/* %{buildroot}/%{_bindir}
mv %{buildroot}/%{ruby_sbindir}/* %{buildroot}/%{_sbindir}
mv %{buildroot}/%{ruby_mandir}/* %{buildroot}/%{_mandir}
